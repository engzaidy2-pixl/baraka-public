-- =====================================================
-- Baraka for Associations — schema.sql (المخطط الأساسي للمنصة)
-- الإصدار: v3.1 (v3.0 + توثيق مخططات الوحدات وقاعدة الترحيلات)
...
--
-- ─────────────────────────────────────────────────────
-- 📌 تسوية الانحراف (2026-09-25) — نتائج فحص الكود مقابل هذا المخطط
-- ─────────────────────────────────────────────────────
-- 1) لا انحراف في الأعمدة: app.js / auth.js / modules.js / ui.js تقرأ من ثلاثة
--    جداول فقط — profiles وsettings وpermissions (بـ select('*')) — وكل أعمدة
--    الجداول الثلاثة معرَّفة هنا، والدالتان المستدعيتان عبر RPC
--    (get_email_by_username وget_quick_login_users) معرَّفتان في القسمين 2.b و2.c.
--
-- 2) جداول معرَّفة ولا يقرأها الكود بعد (محفوظة عمدًا — طبقة الضبط القادمة):
--      workspaces, module_groups, modules, module_tabs, topbar_icons,
--      app_settings, reports_catalog, warehouses, branches, departments, projects
--    السبب: docs/modules.js ما زال يحمل قائمة الوحدات/التبويبات مكتوبة في الكود
--    (مطابقة لـ seed.sql)، ولم يُربط بعد بهذه الجداول. هذا «تأخير تفعيل» وليس
--    تعارضًا، لذا لم تُحذف — لكن أي تعديل عليها يجب أن يمرّ بـ seed.sql أولًا.
--
-- 3) جداول التشغيل الخاصة بوحدة شؤون العضوية ليست في هذا الملف، بل في
--    docs/modules/membership/schema.sql (انظر القسم 16 لترتيب التنفيذ).
--    الانحرافات التي رُصدت هناك (employers.scope، deduction_batches.payment_method،
--    قيود CHECK الأضيق من الواجهة، NOT NULL على جهة خصم الدفعة…) عولجت في
--    الإصدار v2.0 من مخطط الوحدة وفي migrations/2026-10-01-entity-linkage.sql.
-- =====================================================
...
-- =====================================================
-- 16) مخططات الوحدات وقاعدة الترحيلات (Module Schemas & Migrations)
-- هذا الملف = المخطط الأساسي للمنصة (المستخدمون، المساحات، الوحدات، الصلاحيات).
-- جداول التشغيل الخاصة بكل وحدة تعيش في مجلد الوحدة، وتُنفَّذ بعد هذا الملف.
-- =====================================================

-- 16.1) مخططات الوحدات الحالية
--   docs/modules/membership/schema.sql (v2.0) — وحدة شؤون العضوية:
--     employers                 جهات العمل (+scope +has_deduction_entities)
--     deduction_entities        جهات الخصم (+employer_id علاقة 1:N مباشرة، +is_self صف ذاتي)
--     employer_deduction_entities  VIEW توافقي (كان جدول many-to-many — أُسقط في 2026-10-01)
--     membership_types / membership_fees / members / subscriptions / fee_adjustments
--     deduction_batches / deduction_batch_items
--     + دوال الربط: ensure_container_employer(), ensure_self_deduction_entity(id),
--       ensure_all_self_deduction_entities(), sync_employer_deduction_flags()
--     + مشغّلات: الصف الذاتي لجهة العمل، ضم الجهة اليتيمة للحاوية «جهات حكومية»،
--       نقل الأعضاء للصف الذاتي، تكامل جهة خصم العضو مع جهة عمله.

-- 16.2) ترتيب التنفيذ للتنصيبة الجديدة
--   1) docs/schema.sql                          ← هذا الملف
--   2) docs/seed.sql                            ← المساحة والوحدات والصلاحيات
--   3) docs/modules/membership/schema.sql       ← جداول الوحدة + بياناتها الأولية (§15)

-- 16.3) قاعدة الترحيلات (مجلد migrations/ في جذر المستودع)
--   كل تغيير على مخطط قائم يُكتب في ملف مستقل باسم <YYYY-MM-DD>-<slug>.sql
--   ويُنفَّذ مرة واحدة على كل بيئة (الملفات idempotent — آمنة عند إعادة التنفيذ).
--
--   migrations/2026-10-01-entity-linkage.sql
--       المرحلة 0 (تسوية الانحراف D1–D11) + المرحلة 1 (الربط المباشر L1–L7):
--       نقل العلاقة من many-to-many إلى 1:N، الصف الذاتي، حاوية الجهات المستقلة،
--       ربط الأعضاء بجهات الخصم، وتصحيح قيود CHECK / NOT NULL / UNIQUE الناقصة.
--   migrations/2026-10-01-entity-linkage.verify.sql
--       35 فحصًا (مجسّات انحراف + قيود ثابتة + اختبارات سلوكية) تُثبت التطبيق الفعلي.
--       ⚠️ يُنفَّذ على نسخة تجريبية لأنه يُدرج صفوف اختبار مؤقتة ثم يحذفها.

-- =====================================================
-- نهاية schema.sql v3.1
-- =====================================================
